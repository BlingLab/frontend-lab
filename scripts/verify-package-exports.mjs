...
await mustExist("루트 import", join(uiDir, packageJson.exports["."].import));
await mustExist("루트 type", join(uiDir, packageJson.exports["."].types));
await mustExist("카탈로그 import", join(uiDir, packageJson.exports["./components/catalog"].import));
await mustExist("카탈로그 type", join(uiDir, packageJson.exports["./components/catalog"].types));
...
for (const component of componentCatalog) {
  ...
  await mustExist(`${component.name} type`, join(entryBase, "index.d.ts"));
}

const ui = await import("../packages/ui/dist/index.js");
for (const component of componentCatalog) {
  if (!(component.name in ui)) {
    failures.push(`${component.name} 루트 export가 없습니다.`);
  }
}
...
console.log(`패키지 export 검증 완료: ${componentCatalog.length}개 컴포넌트 entry.`);
