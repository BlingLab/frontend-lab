...
if (!version || !/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version)) {
  console.error("사용법: npm run release:prepare -- 0.2.0");
  process.exit(1);
}
...
if (!changelog.includes("## Unreleased")) {
  console.error("CHANGELOG.md에 Unreleased 섹션이 없습니다.");
  process.exit(1);
}
...
console.log(`릴리즈 ${version} 준비 완료. npm install --package-lock-only를 실행해 lockfile을 갱신하세요.`);
