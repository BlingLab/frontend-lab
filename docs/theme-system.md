...
- 기본값은 `normal`입니다. 별도 설정이 없으면 `:root`가 `normal` semantic token을 제공합니다.
- 컴포넌트 CSS는 `--ds-color-action-primary-bg`, `--ds-color-bg-surface`, `--ds-state-selected-bg` 같은 semantic token만 사용합니다.
- 테마는 primitive token보다 semantic token을 재정의합니다. 그래야 Button, DataGrid, Dialog처럼 서로 다른 컴포넌트가 같은 색상 체계를 공유합니다.
- 테마는 앱 전체에도, 특정 product 영역에도 적용할 수 있습니다.

## 제공 테마

| 테마| 용도| 선택자|
| --- | --- | --- |
| `normal` | 기본 제품 UI입니다.| `[data-ds-theme="normal"]`, `.ds-theme-normal` |
| `ocean` | 청록 계열 액션 색상을 쓰는 제품 영역입니다.| `[data-ds-theme="ocean"]`, `.ds-theme-ocean` |
| `forest` | 녹색 계열 업무 화면에 맞춘 제품 영역입니다.| `[data-ds-theme="forest"]`, `.ds-theme-forest` |
| `dark` | 어두운 배경과 높은 대비가 필요한 영역입니다.| `[data-ds-theme="dark"]`, `.ds-theme-dark` |

## NORMAL과 DARK 비교

문서 앱의 `테마 시스템

- `NORMAL`은 밝은 surface, 파란 primary action, 낮은 그림자를 기준으로 합니다.
- `DARK`는 어두운 surface, 밝은 primary action, 높은 대비의 border와 focus ring을 기준으로 합니다.
- 두 패널은 같은 `TextField`, `Alert`, `Badge`, `Button`을 사용하므로 component CSS 변경 없이 theme token 차이만 확인할 수 있습니다.
...
- surface와 canvas 대비가 충분한지 확인합니다.
- primary action의 default, hover, active가 단계적으로 구분되는지 확인합니다.
- focus ring이 밝은 배경과 어두운 배경 모두에서 보이는지 확인합니다.
- selected, hover, active state가 component별 자체 색상 없이도 읽히는지 확인합니다.
- feedback color는 danger, warning, success의 의미가 유지되는지 확인합니다.
