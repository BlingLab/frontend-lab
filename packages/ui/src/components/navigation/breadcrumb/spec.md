...
- `current`: 상태는 시각 스타일과 접근성 신호가 함께 유지되어야 합니다. / This state must keep visual styling and accessibility signals aligned.
- `link`: 상태는 시각 스타일과 접근성 신호가 함께 유지되어야 합니다. / This state must keep visual styling and accessibility signals aligned.
...
- 기준 문서 / Reference: [https://www.w3.org/WAI/ARIA/apg/patterns/breadcrumb/](https://www.w3.org/WAI/ARIA/apg/patterns/breadcrumb/)
...
- 이 컴포넌트는 `nav with ordered list` primitive를 기준으로 구현합니다. / This component is implemented around the `nav with ordered list` primitive.
