# SideNav 컴포넌트 / SideNav Component

제품 workspace를 위한 sectioned vertical navigation입니다. / Sectioned vertical navigation for product workspaces.
...
- 기본 primitive는 `nav with grouped lists`입니다. / Base primitive is `nav with grouped lists`.
...
- 구현 파일은 `side-nav.tsx`, public entry는 `index.ts`입니다. / Implementation lives in `side-nav.tsx`, and the public entry is `index.ts`.
