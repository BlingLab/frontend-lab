# SideNav 명세 / SideNav Spec

## 목적 / Purpose

그룹화된 destination과 현재 위치를 sidebar 안에서 유지해야 할 때 사용합니다. / Use for application sidebars with grouped destinations and persistent current state.

## API 표면 / API Surface

- public component: `SideNav`
- folder slug: `side-nav`
- category: `navigation`
...
- `expanded`: 상태는 시각 스타일과 접근성 신호가 함께 유지되어야 합니다. / This state must keep visual styling and accessibility signals aligned.
- `collapsed`: 상태는 시각 스타일과 접근성 신호가 함께 유지되어야 합니다. / This state must keep visual styling and accessibility signals aligned.
...
- 이 컴포넌트는 `nav with grouped lists` primitive를 기준으로 구현합니다. / This component is implemented around the `nav with grouped lists` primitive.
