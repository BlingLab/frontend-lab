...
관련 그룹 사이를 시각적으로 구분하는 separator입니다. / Visual separator between related groups.
...
- 기본 primitive는 `hr or role='separator'`입니다. / Base primitive is `hr or role='separator'`.
...
- 구현 파일은 `divider.tsx`, public entry는 `index.ts`입니다. / Implementation lives in `divider.tsx`, and the public entry is `index.ts`.
