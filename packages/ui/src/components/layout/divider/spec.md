# Divider 명세 / Divider Spec

## 목적 / Purpose

새 container를 만들지 않고 인접한 영역의 경계를 표현할 때 사용합니다. / Use to separate adjacent sections without introducing a new container.
...
- `horizontal`: 상태는 시각 스타일과 접근성 신호가 함께 유지되어야 합니다. / This state must keep visual styling and accessibility signals aligned.
- `vertical`: 상태는 시각 스타일과 접근성 신호가 함께 유지되어야 합니다. / This state must keep visual styling and accessibility signals aligned.
...
- 이 컴포넌트는 `hr or role='separator'` primitive를 기준으로 구현합니다. / This component is implemented around the `hr or role='separator'` primitive.
