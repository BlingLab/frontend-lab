...
- `static`: 상태는 시각 스타일과 접근성 신호가 함께 유지되어야 합니다. / This state must keep visual styling and accessibility signals aligned.
- `animated`: 상태는 시각 스타일과 접근성 신호가 함께 유지되어야 합니다. / This state must keep visual styling and accessibility signals aligned.
...
- 이 컴포넌트는 `div with aria-hidden when decorative` primitive를 기준으로 구현합니다. / This component is implemented around the `div with aria-hidden when decorative` primitive.
