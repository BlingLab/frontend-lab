...
- `entering`: 상태는 시각 스타일과 접근성 신호가 함께 유지되어야 합니다.
- `open`: 상태는 시각 스타일과 접근성 신호가 함께 유지되어야 합니다.
- `exiting`: 상태는 시각 스타일과 접근성 신호가 함께 유지되어야 합니다.
- `dismissed`: 상태는 시각 스타일과 접근성 신호가 함께 유지되어야 합니다.
...
- 이 컴포넌트는 `status region with dismiss control` primitive를 기준으로 구현합니다.
