...
- `off`: 상태는 시각 스타일과 접근성 신호가 함께 유지되어야 합니다. / This state must keep visual styling and accessibility signals aligned.
- `on`: 상태는 시각 스타일과 접근성 신호가 함께 유지되어야 합니다. / This state must keep visual styling and accessibility signals aligned.
...
- 기준 문서 / Reference: [https://www.w3.org/WAI/ARIA/apg/patterns/switch/](https://www.w3.org/WAI/ARIA/apg/patterns/switch/)
...
- 이 컴포넌트는 `button or input checkbox with switch semantics` primitive를 기준으로 구현합니다. / This component is implemented around the `button or input checkbox with switch semantics` primitive.
